...
First, add the below code snippet to your existing test suite in [`spec/recording-items.spec.ts`](https://github.com/serenity-js/serenity-js-playwright-test-template/blob/main/spec/recording-items.spec.ts)
...
and instead helps you focus on [**actors**](/api/core/class/Actor)—**people** and **processes** interacting with
...
or a website crawler bot scanning a web UI and receiving a different version than a regular user. You can also use actors to represent
...
From the implementation perspective, the [Serenity/JS Playwright Test](/api/playwright-test)
...
- **Tasks** are **composites of other activities**, created to capture **business domain vocabulary**
  and help to make steps in your test scenario reflect **steps in your business workflow**. Examples here might include
...
:::info Did you know?
The task decomposition model implemented by Serenity/JS is based on Hierarchical Task Analysis, a technique typically used in User-Centred Design.
...
Serenity/JS's activity-based code reuse model also means that as you evolve the vocabulary of your domain-specific tasks
...
Serenity/JS's activity-based programming model applies to performing [assertions](/handbook/design/assertions) just as well as it applies
...
In fact, Serenity/JS's assertion to [`Ensure`](/api/assertions/class/Ensure) is just another interaction
...
Congratulations! 🥳 You've just learnt how to write, run, and debug your first Serenity/JS test scenarios!
...
- Make sure to [give Serenity/JS a ⭐️ on GitHub](https://github.com/serenity-js/serenity-js) and leave a comment or 👍 if you've enjoyed this tutorial!
